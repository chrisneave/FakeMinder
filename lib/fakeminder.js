...
FakeMinder.prototype.handleProtectedRequest = function(req, res, current_session) {
    ...
    if (current_session.auth_headers) {
      for (var auth_header in current_session.auth_headers) {
        req.headers[auth_header] = current_session.auth_headers[auth_header];
      }
    }
...
FakeMinder.prototype.handleRequest = function(req, res, done) {
  ...
  if (req.url.indexOf(self.getAbsoluteUrl('protected')) !== -1) {
    if (!self.handleProtectedRequest(req, res, current_session)) {
      done(false);        
      return;
    }
  }
