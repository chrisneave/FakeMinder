  ...
  if (_.isUndefined(formcred_session) === false) {
    switch (formcred_session.status) {
      ...
      case Model.FormCredStatus.bad_login:
        res.statusCode = 302;
        res.setHeader('Location', self.getAbsoluteUrl('bad_login'));
        res.end();
        return;
      ...
      case Model.FormCredStatus.bad_password:
        ...
        res.statusCode = 302;
        res.setHeader('Location', self.getAbsoluteUrl('bad_password'));
        res.end();
        return;
    }
  } else {
...
FakeMinder.prototype.logoffHandler = function(req, res, next) {
  ...
  if (req.url !== self.getAbsoluteUrl('logoff')) {
    ...
    cookieJar.set(self.SESSION_COOKIE, current_session.session_id, {'domain':'localhost'});
    next();
    return;
  }
...
FakeMinder.prototype.getAbsoluteUrl = function(url_type) {
  return url.resolve(this.config.target_site.root, this.config.target_site.urls[url_type]);
};
