...
FakeMinder.prototype.handleRequest = function(req, res) {
  ...
  var session_expires_if_before = this.getSessionExpiresBefore();
  ...
  if (req.url.indexOf(this.getAbsoluteUrl('protected')) !== -1) {
    if (!existing_session || existing_session_expiration < session_expires_if_before) {
      res.statusCode = 302;
    ...
    existing_session_expiration = this.getSessionExpiration();
